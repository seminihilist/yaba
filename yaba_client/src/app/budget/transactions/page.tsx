...
// import { Item, Section, ItemMap, Transaction, TransactionGroup } from "@/lib/types";
// import TransactionGroupComponent from "@/app/budget/[budgetId]/transactions/components/TransactionGroupComponent";
//
...
//
// 	const [ntdSelectedItem, setNtdSelectedItem] = useState(-1); // set to the ID of the item
//
...
// 	const sectionsAndIDs: Array<[string, Section]> = Object.entries(useAppSelector((state: RootState) => state.budget.sections))
//
// 	// IDs become strings in objects, so we need to worry about that
// 	const idToItemMap: ItemMap = useAppSelector((state: RootState) => state.budget.items)
// 	const itemsAndIDs: Array<[string, Item]> = Object.entries(idToItemMap)
//
...
//
// 							const [id, item]: [string, Item] = itemsAndIDs.filter(([idStr, item]) => item.id === ntdSelectedItem)[0]; // There should be only one match, so we get the first
//
// 							const itemAmountRemaining = item.amount -
// 								transactions.filter((transaction) => transaction.itemID === item.id)
...
// 														const item = idToItemMap[itemID];
//
// 														console.log(transactions.filter((transaction) => transaction.itemID === item.id))
//
// 														const itemAmountRemaining = item.amount -
// 															transactions.filter((transaction) => transaction.itemID === item.id)
// 																.reduce((acc, transaction) => acc + transaction.amount, 0)
